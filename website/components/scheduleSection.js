...
export default function ScheduleSection({ events, config }) {
  ...
  return (
    <article>
      <div className='relative w-full py-20 lg:px-10 min-h-[10vh] text-white' id='schedule'>
        <div className="absolute inset-0 overflow-hidden">
          <img src="./schedule-ornament-top.svg" width="991" className="absolute top-10 left-1/2 transform -translate-x-10" />
        </div>
        <div className="relative z-10">
          <div className="container mx-auto max-w-7xl pb-10 px-6 lg:px-0">
            <header>
              <h1 className="text-h1 block">Schedule</h1>
              <div className="text-body-lg mb-4">
                {config.schedule?.description && <Markdown>{config.schedule.description}</Markdown>}
              </div>
            </header>
            <ScrollContainer innerRef={scrollContainer} className="scroll-container py-10 mx-auto max-w-7xl">
